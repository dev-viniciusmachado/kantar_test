    ...
    IEnumerable<IDiscountPolicy> discountPolicies) : IRequestHandler<DeleteItemCommand, Guid>
{
    public async Task<Guid> Handle(DeleteItemCommand request, CancellationToken cancellationToken)
    {
        ...
        var itemsToRemove = basket.RemoveItems(product, discountPolicies, discount);
        
        if (!itemsToRemove.Any())
        {
            logger.LogInformation("No items found for product ID {ProductId} in basket ID {BasketId}", request.ProductId, request.BasketId);
            throw new KeyNotFoundException($"No items found for product ID {request.ProductId} in basket ID {request.BasketId}");
        }
